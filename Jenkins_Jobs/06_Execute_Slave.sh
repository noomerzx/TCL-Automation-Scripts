...
# Initial all vairbale
initialVariablesForRunningOS
Section=${_SECTIONS}

# Ensure that required folder exists, and cd to target path
echo "[Shell] Change Path to ${_AUTOMATE_PATH}${_OS}RunAT${_OS}${_PKG_NAME}${_OS}${Section}${_OS}RunAT"
mkdir -p ${_AUTOMATE_PATH}${_OS}RunAT${_OS}${_PKG_NAME}${_OS}${Section}${_OS}RunAT
cd ${_AUTOMATE_PATH}${_OS}RunAT${_OS}${_PKG_NAME}${_OS}${Section}${_OS}RunAT
...
CONFIG_JAVA="06_Execute_Slave_${PLAT_CURRENT}_Config.txt"
