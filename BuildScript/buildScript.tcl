...
#catch {package require ZION 1.0}
package require ZION 1.0
...
proc prepareConfig {outputFolderName rootFolder} {
	...
	if { ${platformName} =="W"} {
		...
		if {[catch {cd ${outputFolderName}}]} {
			...
			sendMessage "\[INFO\] ::: Pass -> cd $outputFolderName"
...
proc buildInWindow { folder application platform fileType buildMode} {
	...
	if {[regexp {([1-9] succeeded, 0 failed, (0 up-to-date, )?0 skipped)|(1 up-to-date)} ${result} ]} {
		sendMessage "\[INFO\] ::: Pass -> Build Finish"
	}
...
proc buildExample { folder application { platform "-" } { fileType "-" } { buildMode "-" }  } {
	...
	if { ${platformName} == "W"} {
		...
		puts ">>>>>>>>>>> build result UNIX : $buildResultUnix"
	}
	...
	if {[regexp {01_PackageVerification} ${rootFolder}]} {
		...
		puts ">>>>>>>>>>> Update Test result to zion "
		puts ">>>>>>>>>>> ExampleName : ${exampleName}"
		puts ">>>>>>>>>>> Platform : ${platform}"
		puts ">>>>>>>>>>> Result : ${buildExampleResult}"
	} else {
...
# initial /export/home/administrator/Packages/RFACPP760_CANRRG "./config-zion.txt"							#
# buildExample Examples_noom/Consumer Consumer RH5L_64_412_SH												#
#																											#
#############################################################################################################	
